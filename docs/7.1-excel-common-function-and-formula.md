...
```excel
=IF(E2="Excellent","Eligible","Not Eligible")
```
...
```excel
=IF(OR(E2="Outstanding",B2="IT"),"Qualifies","No")
```
...
```excel
=AVERAGEIFS(D2:D9,E2:E9,"Excellent")
```
...
```excel
=COUNTIFS(B2:B9,"IT",E2:E9,"Excellent",G2:G9,"Yes")
```
...
```excel
=IFERROR(VLOOKUP("Unknown",B2:D9,3,FALSE),"Not Available")
```
...
```excel
=INDEX(D2:D9,MATCH("Hassan Ali",B2:B9,0))
```
...
```excel
=IFS(D2>55000,"Premium",D2>=45000,"Standard",TRUE,"Basic")
```
...
```excel
=SUMPRODUCT((D2:D9>50000)*(E2:E9="Excellent")*D2:D9*0.1)
```
...
```excel
=IF(AND(E2="Excellent",G2="Yes"),D2*0.15,IF(E2="Excellent",D2*0.1,IF(AND(E2="Good",G2="Yes"),D2*0.05,0)))
```
